...
from ankigen.src.study.sources.text import iter_interests_from_text, iter_samples_from_text
...
class AnkiBuilder:
    ...
    def with_firefox_wiktionary_interests(self, path: str, past_day_count: float) -> Self:
    ...
    def with_epub_samples(self, glob: str) -> Self:
    ...
    def with_text_samples(self, glob: str) -> Self:
        def func() -> Iterable[Sample]:
            for path in _iter_paths(glob):
    ...
    def with_text_interests(self, glob: str) -> Self:
        def func() -> Iterable[Interest]:
            for path in _iter_paths(glob):
                self._log_func(f"loading interests (text) from '{path}' ...")
                yield from iter_interests_from_text(self._context, path)
        self._interest_generators.append(func)
        return self

    def with_kaikki_samples(self, path: str) -> Self:
        def func() -> Iterable[Sample]:
